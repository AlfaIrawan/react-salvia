...
export function TodoListPanel({ panelContainerEl }: TodoListPanelProps = {}) {
  ...
  const filteredTodos = useMemo(() => {
    ...
    if (filter === 'done') return todos.filter((t) => t.is_completed)
    return todos
  }, [todos, filter])
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewDueDate('')
      setNewPriority(null)
      window.dispatchEvent(new CustomEvent('salvia-todos-changed'))
      await loadTodos()
    } catch (e) {
  ...
  return (
    <div className="relative flex flex-col gap-6 min-h-full px-5" onClick={handlePanelClick}>
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* Filter row: segmented control + priority pill + date */}
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex p-0.5 rounded-lg bg-slate-100 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700/50">
              ...
              {sectionLabels.map(({ key, label }) => {
                ...
                return (
                  <Fragment key={key}>
                    <div
                      className={cn(
                        'sticky top-0 z-[1] flex items-center gap-3 py-3 first:pt-0 bg-gradient-to-b from-transparent via-white/80 to-white dark:via-slate-900/80 dark:to-slate-900 backdrop-blur-sm',
