...
const BASE_URL = import.meta.env.VITE_NOTIFICATION_API_URL ?? 'http://localhost:8700'

/** App GUID for Salvia only (notification service — jangan dipakai Tilia/app lain). */
export const SALVIA_APP_ID = '00000000-0000-0000-0000-000000000004'
...
export async function fetchNotifications(params: NotificationListParams & {
  ...
  const res = await fetch(`${BASE_URL}/v1/notifications?${sp}`)
...
export async function fetchUnreadCount(params: { app_id: string; user_id: string }): Promise<UnreadCountResponse> {
  ...
  const res = await fetch(`${BASE_URL}/v1/notifications/unread-count?${sp}`)
...
export async function markNotificationRead(
  ...
  sp.set('user_id', params.user_id)
  sp.set('is_read', 'true')
  const res = await fetch(`${BASE_URL}/v1/notifications/${id}/read?${sp}`, {
    method: 'PATCH',
...
export async function markAllNotificationsRead(params: {
  ...
  sp.set('app_id', params.app_id)
  sp.set('user_id', params.user_id)
  const res = await fetch(`${BASE_URL}/v1/notifications/mark-all-read?${sp}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  })
  if (!res.ok) {
...
export function notifyEvent(params: {
  ...
  createNotification({
    app_id: SALVIA_APP_ID,
    user_id: session.user.id,
    type_code: params.type_code,
    title: params.title,
    body: params.body ?? null,
    link_url: params.link_url ?? null,
    created_from: 'salvia-frontend',
  }).catch(() => {})
}
