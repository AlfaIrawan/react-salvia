...
const BASE_URL = import.meta.env.VITE_TODO_API_URL ?? 'http://localhost:8650'

/** App GUID for Salvia only (todo service app_id — jangan dipakai app lain). */
export const SALVIA_TODO_APP_ID = '00000000-0000-0000-0000-000000000004'
...
export async function fetchTodos(params?: {
  ...
  const url = new URL(`${BASE_URL}/v1/todos`)
...
export async function createTodo(payload: TodoCreatePayload): Promise<TodoItem> {
  const res = await fetch(`${BASE_URL}/v1/todos`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...payload,
      app_id: payload.app_id ?? SALVIA_TODO_APP_ID,
    }),
  })
...
export async function updateTodo(id: string, payload: TodoUpdatePayload): Promise<TodoItem> {
  const res = await fetch(`${BASE_URL}/v1/todos/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteTodo(id: string): Promise<void> {
  const res = await fetch(`${BASE_URL}/v1/todos/${id}`, { method: 'DELETE' })
